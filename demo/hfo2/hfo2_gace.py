...
class HFO2App(AbstractApp):
    ...
    def ind_to_elis(self, individual):
        ...
        """
        def get_ele(x):
            if x < self.params_config_dict['NB_DEFECT']:
                return self.params_config_dict['SECOND_ELEMENT']
            else:
                return self.params_config_dict['FIRST_ELEMENT']

        element_lis = [get_ele(i) for i in individual]
        return element_lis
    ...
    def evalEnergy(self, individual):
        ...
        if k in self.ENERGY_DICT.keys():
            ...
            energy = (energy - self.params_config_dict['PERFECT_HFO2'] +
                      ...
                      self.params_config_dict['MU_OXYGEN']) / self.params_config_dict['NB_DEFECT']
    ...
    def single_run(self, mission_name, repeat_iter):
        ...
        res = gaceGA(pop, self.toolbox, cxpb=0.5, ngen=90, stats=stats, halloffame=hof,
                     verbose=True, checkpoint=checkpoint_fname.format(mission_name, repeat_iter))
    ...
    def multiple_run(self, mission_name, repeat_iters):
        ...
        mission_name : str
            A string used to represent the name of current running.
        repeat_iters : int
            How many times should a simulation repeat for statistic error(different ground-state in
            different running iterations with identical parameter setting).
        ...
        global_min_idx = np.argmin(all_min)
        ...
        np.savetxt(s_fname.format(mission_name), all_min, fmt='%.3f')
...
class Runner(AbstractRunner):
    ...
    def print_gs(self):
        ...
        if DEBUG:
            if self.iter_idx == 1:
                print('For the 1st iteration there are three structures needed '
                      'to be calculated by DFT:')
                print('0_3_7_9_10_24_32_33_34_35_40_42_53_55_56_61, \n'
                      ...
                      'new `eci.out`.')
            if self.iter_idx == 2:
                print('For the 2nd iteration, there are two structures needed to be'
                      'calculated by DFT:')
                print('0_8_15_16_34_35_42_43_53_55_56_57_58_60_61_63, \n'
                      ...
                      'new `eci.out`.')
            if self.iter_idx == 3:
                print('For the 3rd iteration, there are two structures needed to be'
                      'calculated by DFT:')
                print('1_5_8_14_18_21_30_31_37_38_41_43_46_54_57, \n'
                      ...
                      'new `eci.out`.')
            if self.iter_idx == 4:
                print('For the 4th iteration, there are six structures needed to be'
                      'calculated by DFT:')
                print('2_3_9_23_33_38_49_62, \n'
                      ...
                      'new `eci.out`.')
            if self.iter_idx == 5:
                print('In our previous calculation, for the last iteration, all '
...
class HfO2EleIndv(EleIndv):
    ...
    def __eq__(self, other):
        return np.abs(self.ce_energy - other.ce_energy) < 0.001
    ...
    def dft_energy(self, iters=None, vasp_cmd=None, update_eci=True):
                ...
                for f in os.listdir(pre_cal_dir):
                    if os.path.isdir(os.path.join(pre_cal_dir, f)):
                        ...
                        if '{:.6}'.format(pre_e) == '{:.6}'.format(cur_e):
                            pre_dft_energy = float(np.loadtxt(os.path.join(pre_cal_dir,
                                                                           f, 'energy')))
                            return True
            else:
                return False
        ...
        # self.run_vasp()
        cur_dft_energy = float(np.loadtxt('energy', dtype=float))
        os.chdir(cur_dir)
        ...
        if update_eci:
            ...
            cal_name_in_next_atat_path = os.path.join(
                next_atat_path, 'dft_' + basename + '_' + os.path.basename(cal_dir))
