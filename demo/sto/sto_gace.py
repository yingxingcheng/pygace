...
class STOApp(AbstractApp):
    ...
    def __init__(self, ce_site=1, ce_dirname='./data/iter0', ele_1st='Ti_sv', ele_2nd='Nb_sv',
    ...
    def multiple_run(self, ce_iter, ga_iters):
        ...
        min_idx = np.argmin(all_min)

        s_fname = os.path.join(self.params_config_dict['TEST_RES_DIR'], '{0}.txt')
        np.savetxt(s_fname.format(ce_iter), all_min, fmt='%.3f')
    ...
    def create_dir_for_DFT(self, task_fname='./DFT_task.dat'):
        ...
        ----------
        task_fname : str
            Name of file restoring the directory in which DFT task should be performed.
