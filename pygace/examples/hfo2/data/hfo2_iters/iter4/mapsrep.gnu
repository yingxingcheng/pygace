...
#unset key
 splot  './gs_lines.out' u (0.5*(2*$2+$3)/($1+$2+$3)):(sqrt(3)/2*($3)/($1+$2+$3)):($5) t "known gs" w lp pt 4, \
	...
	'./predstr.out' u (0.5*(2*$2+$3)/($1+$2+$3)):(sqrt(3)/2*($3)/($1+$2+$3)):($4) t "predicted" w p pt 0
pause -1
set title "Calculated Energies"
 splot  './gs_lines.out' u (0.5*(2*$2+$3)/($1+$2+$3)):(sqrt(3)/2*($3)/($1+$2+$3)):($4) t "known gs" w lp pt 4, \
	'./fit.out' u (0.5*(2*$2+$3)/($1+$2+$3)):(sqrt(3)/2*($3)/($1+$2+$3)):($4) t "known str" w p pt 1
pause -1
set title "Calculated and Fitted Energies"
 splot  './fit.out' u (0.5*(2*$2+$3)/($1+$2+$3)):(sqrt(3)/2*($3)/($1+$2+$3)):($4) t "calculated" w p pt 1, \
	'./fit.out' u (0.5*(2*$2+$3)/($1+$2+$3)):(sqrt(3)/2*($3)/($1+$2+$3)):($5) t "fitted" w p pt 3 
pause -1
set border
set xlabel "diameter"
set xtics
...
set nokey
plot \
'fit.out' u ($4):($5) w p pt 1,x w l lt 0
pause -1
